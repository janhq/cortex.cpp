...
// Check if the template supplied via "--chat-template" is supported or not.
// Returns true if it's valid
inline bool verify_custom_template(const std::string &tmpl) {
  llama_chat_message chat[] = {{"user", "test"}};
  std::vector<char> buf(1);
  int res = llama_chat_apply_template(nullptr, tmpl.c_str(), chat, 1, true,
                                      buf.data(), buf.size());
  return res >= 0;
}
...
inline std::string format_chat(const struct llama_model *model,
  ...
  size_t alloc_size = 0;
  // vector holding all allocated string to be passed to
  ...
  int32_t res = llama_chat_apply_template(
      model, ptr_tmpl, chat.data(), chat.size(), true, buf.data(), buf.size());

  // if it turns out that our buffer is too small, we resize it
  if ((size_t)res > buf.size()) {
    buf.resize(res);
    res = llama_chat_apply_template(model, ptr_tmpl, chat.data(), chat.size(),
