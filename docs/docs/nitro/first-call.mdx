...
To make your first inference request using Nitro.
...
<Tabs>
  <TabItem value="usingNitro" label="Using Nitro">
    <p>To set up using Nitro, open the terminal and run:</p>
  ...
  <TabItem value="usingOpenAIAPI" label="Using OpenAI API">
    <p>For setting up using the OpenAI API, open the terminal and execute:</p>
    <pre><code className="language-zsh">{`curl https://api.openai.com/v1/chat/completions \
  -H "Content-Type: application/json" \
