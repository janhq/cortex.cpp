...
const { stopModel, runModel, loadLLMModel, validateModelStatus, chatCompletion } = nitro
...
const createProgressReporter = (name: string) => (stream: Promise<Buffer> & Duplex) => stream.on(
  ...
  (progress: { transferred: any; total: any; percent: number }) => {
    ...
    process.stdout.write(`\r\x1b[K[${name}] ${progress.transferred}/${progress.total} ${Math.floor(progress.percent * 100)}%...`);
  }).on('end', () => {
...
const downloadModelHook = (modelCfg: any, targetDir: string) => async () => {
  ...
  const downloadedModelFile = fs.readdirSync(TEST_ASSETS_PATH).find((fname) => fname.match(/.*\.gguf/ig))
...
describe('Manage nitro process', () => {
  ...
  test('chat completion',
    async () => {
      ...
      console.log(`[Streamed response] ${JSON.stringify(streamedContent, null, 2)}`)
