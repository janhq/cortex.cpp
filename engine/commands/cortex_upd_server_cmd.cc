#include "cortex_upd_server_cmd.h"
...
void CortexUpdServerCmd::Exec(const std::string& v) {
...
bool CortexUpdServerCmd::GetStable(const std::string& v) {
  ...
  auto src =
      std::filesystem::temp_directory_path() / "cortex" / GetCortexServerBinary();
  auto dst = executable_path / GetCortexServerBinary();
  utils::ScopeExit se([]() {
...
bool CortexUpdServerCmd::GetBeta(const std::string& v) {
  auto system_info = GetSystemInfoWithUniversal();
  CTL_INF("OS: " << system_info->os << ", Arch: " << system_info->arch);
...
bool CortexUpdServerCmd::HandleGithubRelease(const nlohmann::json& assets,
...
bool CortexUpdServerCmd::GetNightly(const std::string& v) {
  auto system_info = GetSystemInfoWithUniversal();
  CTL_INF("OS: " << system_info->os << ", Arch: " << system_info->arch);
