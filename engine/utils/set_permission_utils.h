...
[[nodiscard]] inline bool SetExecutePermission(
    const std::filesystem::path& filePath, bool ownerOnly = false) noexcept {
  try {
    ...
    if (ownerOnly) {
      ...
      new_perms &= ~(std::filesystem::perms::group_exec |
...
SetExecutePermissionsRecursive(const std::filesystem::path& directoryPath,
                               bool ownerOnly = false,
                               bool skipDirectories = true) {
  std::vector<std::filesystem::path> modifiedFiles;
  modifiedFiles.reserve(
      100);  // Reserve space to prevent frequent reallocations
