...
inline cpp::result<std::unique_ptr<ModelMetadata>, std::string>
ReadGgufMetadata(const std::filesystem::path& path) {
  ...
  auto metadata_ptr = std::make_unique<ModelMetadata>();
  ...
  {
    metadata_ptr->tokenizer = std::make_unique<GgufTokenizer>();
