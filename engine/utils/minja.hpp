...
class Value : public std::enable_shared_from_this<Value> {
 ...
 public:
  ...
  T get() const {
    ...
    throw std::runtime_error("get<T> not defined for this value type: " +
                             dump());
  }
...
struct TextTemplateToken : public TemplateToken {
  std::string text;
  TextTemplateToken(const Location& location, SpaceHandling pre,
                    SpaceHandling post, const std::string& t)
      : TemplateToken(Type::Text, location, pre, post), text(t) {}
};

struct ExpressionTemplateToken : public TemplateToken {
  std::shared_ptr<Expression> expr;
  ExpressionTemplateToken(const Location& location, SpaceHandling pre,
                          SpaceHandling post, std::shared_ptr<Expression>&& e)
      : TemplateToken(Type::Expression, location, pre, post),
        expr(std::move(e)) {}
};

struct IfTemplateToken : public TemplateToken {
  std::shared_ptr<Expression> condition;
  IfTemplateToken(const Location& location, SpaceHandling pre,
                  SpaceHandling post, std::shared_ptr<Expression>&& c)
      : TemplateToken(Type::If, location, pre, post), condition(std::move(c)) {}
};

struct ElifTemplateToken : public TemplateToken {
  std::shared_ptr<Expression> condition;
  ElifTemplateToken(const Location& location, SpaceHandling pre,
                    SpaceHandling post, std::shared_ptr<Expression>&& c)
      : TemplateToken(Type::Elif, location, pre, post),
        condition(std::move(c)) {}
};

struct ElseTemplateToken : public TemplateToken {
  ElseTemplateToken(const Location& location, SpaceHandling pre,
                    SpaceHandling post)
      : TemplateToken(Type::Else, location, pre, post) {}
};

struct EndIfTemplateToken : public TemplateToken {
  EndIfTemplateToken(const Location& location, SpaceHandling pre,
                     SpaceHandling post)
      : TemplateToken(Type::EndIf, location, pre, post) {}
};

struct MacroTemplateToken : public TemplateToken {
  std::shared_ptr<VariableExpr> name;
  Expression::Parameters params;
  MacroTemplateToken(const Location& location, SpaceHandling pre,
                     SpaceHandling post, std::shared_ptr<VariableExpr>&& n,
                     Expression::Parameters&& p)
      : TemplateToken(Type::Macro, location, pre, post),
        name(std::move(n)),
        params(std::move(p)) {}
};

struct EndMacroTemplateToken : public TemplateToken {
  EndMacroTemplateToken(const Location& location, SpaceHandling pre,
                        SpaceHandling post)
      : TemplateToken(Type::EndMacro, location, pre, post) {}
};

struct FilterTemplateToken : public TemplateToken {
  std::shared_ptr<Expression> filter;
  FilterTemplateToken(const Location& location, SpaceHandling pre,
                      SpaceHandling post, std::shared_ptr<Expression>&& filter)
      : TemplateToken(Type::Filter, location, pre, post),
        filter(std::move(filter)) {}
};

struct EndFilterTemplateToken : public TemplateToken {
  EndFilterTemplateToken(const Location& location, SpaceHandling pre,
                         SpaceHandling post)
      : TemplateToken(Type::EndFilter, location, pre, post) {}
};
...
struct ForTemplateToken : public TemplateToken {
  ...
  std::shared_ptr<Expression> condition;
  bool recursive;
  ForTemplateToken(const Location& location, SpaceHandling pre,
                   SpaceHandling post, const std::vector<std::string>& vns,
                   std::shared_ptr<Expression>&& iter,
                   std::shared_ptr<Expression>&& c, bool r)
      : TemplateToken(Type::For, location, pre, post),
        var_names(vns),
...
struct EndForTemplateToken : public TemplateToken {
  EndForTemplateToken(const Location& location, SpaceHandling pre,
                      SpaceHandling post)
...
struct SetTemplateToken : public TemplateToken {
  ...
  SetTemplateToken(const Location& location, SpaceHandling pre,
                   SpaceHandling post, const std::string& ns,
                   const std::vector<std::string>& vns,
                   std::shared_ptr<Expression>&& v)
      : TemplateToken(Type::Set, location, pre, post),
        ns(ns),
        var_names(vns),
        value(std::move(v)) {}
};
...
struct EndSetTemplateToken : public TemplateToken {
  EndSetTemplateToken(const Location& location, SpaceHandling pre,
                      SpaceHandling post)
      : TemplateToken(Type::EndSet, location, pre, post) {}
};

struct CommentTemplateToken : public TemplateToken {
  std::string text;
  CommentTemplateToken(const Location& location, SpaceHandling pre,
                       SpaceHandling post, const std::string& t)
...
class SequenceNode : public TemplateNode {
 ...
 public:
  SequenceNode(const Location& location,
...
class TextNode : public TemplateNode {
 ...
 public:
  TextNode(const Location& location, const std::string& t)
      : TemplateNode(location), text(t) {}
  void do_render(std::ostringstream& out,
...
class ExpressionNode : public TemplateNode {
 ...
 public:
  ExpressionNode(const Location& location, std::shared_ptr<Expression>&& e)
      : TemplateNode(location), expr(std::move(e)) {}
  void do_render(std::ostringstream& out,
                 const std::shared_ptr<Context>& context) const override {
    if (!expr)
...
class IfNode : public TemplateNode {
 ...
 public:
  IfNode(const Location& location,
...
class ForNode : public TemplateNode {
 ...
 public:
  ForNode(const Location& location, std::vector<std::string>&& var_names,
          std::shared_ptr<Expression>&& iterable,
          std::shared_ptr<Expression>&& condition,
          std::shared_ptr<TemplateNode>&& body, bool recursive,
          std::shared_ptr<TemplateNode>&& else_body)
      : TemplateNode(location),
        var_names(var_names),
...
class MacroNode : public TemplateNode {
 ...
 public:
  MacroNode(const Location& location, std::shared_ptr<VariableExpr>&& n,
  ...
  void do_render(std::ostringstream&,
    ...
    auto callable = Value::callable([&](const std::shared_ptr<Context>& context,
                                        ArgumentsValue& args) {
      ...
      for (size_t i = 0, n = args.args.size(); i < n; i++) {
        ...
        if (i >= params.size())
          throw std::runtime_error("Too many positional arguments for macro " +
                                   name->get_name());
        param_set[i] = true;
      ...
      for (auto& [arg_name, value] : args.kwargs) {
        ...
        if (it == named_param_positions.end())
          throw std::runtime_error("Unknown parameter name for macro " +
...
class FilterNode : public TemplateNode {
 ...
 public:
  FilterNode(const Location& location, std::shared_ptr<Expression>&& f,
  ...
  void do_render(std::ostringstream& out,
    ...
    if (!filter_value.is_callable()) {
      throw std::runtime_error("Filter must be a callable: " +
                               filter_value.dump());
    }
...
class SetNode : public TemplateNode {
 ...
 public:
  SetNode(const Location& location, const std::string& ns,
  ...
  void do_render(std::ostringstream&,
    ...
    if (!ns.empty()) {
      if (var_names.size() != 1) {
        throw std::runtime_error(
            "Namespaced set only supports a single variable name");
      }
...
class SetTemplateNode : public TemplateNode {
 ...
 public:
  SetTemplateNode(const Location& location, const std::string& name,
...
class IfExpr : public Expression {
 ...
 public:
  IfExpr(const Location& location, std::shared_ptr<Expression>&& c,
...
class ArrayExpr : public Expression {
 ...
 public:
  ArrayExpr(const Location& location,
            std::vector<std::shared_ptr<Expression>>&& e)
...
class SliceExpr : public Expression {
 public:
  ...
  SliceExpr(const Location& location, std::shared_ptr<Expression>&& s,
            std::shared_ptr<Expression>&& e)
...
class SubscriptExpr : public Expression {
 ...
 public:
  SubscriptExpr(const Location& location, std::shared_ptr<Expression>&& b,
  ...
  Value do_evaluate(const std::shared_ptr<Context>& context) const override {
    ...
    if (auto slice = dynamic_cast<SliceExpr*>(index.get())) {
      auto start =
          slice->start ? slice->start->evaluate(context).get<int64_t>() : 0;
      auto end = slice->end ? slice->end->evaluate(context).get<int64_t>()
                            : (int64_t)target_value.size();
      if (target_value.is_string()) {
        ...
        throw std::runtime_error(
            target_value.is_null()
                ? "Cannot subscript null"
      ...
      if (target_value.is_null()) {
        ...
        throw std::runtime_error("Trying to access property '" +
                                 index_value.dump() + "' on null!");
      }
...
class BinaryOpExpr : public Expression {
 ...
 public:
  BinaryOpExpr(const Location& location, std::shared_ptr<Expression>&& l,
  ...
  Value do_evaluate(const std::shared_ptr<Context>& context) const override {
    ...
    auto do_eval = [&](const Value& l) -> Value {
      if (op == Op::Is || op == Op::IsNot) {
        ...
        if (!t)
          throw std::runtime_error(
              "Right side of 'is' operator must be a variable");
    ...
    for (const auto& arg : this->args) {
      if (auto un_expr = std::dynamic_pointer_cast<UnaryOpExpr>(arg)) {
        if (un_expr->op == UnaryOpExpr::Op::Expansion) {
          ...
          if (!array.is_array()) {
            throw std::runtime_error(
                "Expansion operator only supported on arrays");
          }
...
class MethodCallExpr : public Expression {
 ...
 public:
  MethodCallExpr(const Location& location, std::shared_ptr<Expression>&& obj,
...
class CallExpr : public Expression {
 public:
  std::shared_ptr<Expression> object;
  ArgumentsExpression args;
  CallExpr(const Location& location, std::shared_ptr<Expression>&& obj,
...
class FilterExpr : public Expression {
 ...
 public:
  FilterExpr(const Location& location,
 ...
 private:
    ...
    while (it != end && consumeSpaces() && peekSymbols({"[", "."})) {
      if (!consumeToken("[").empty()) {
        ...
        if (!consumeToken(":").empty()) {
          ...
          index = std::make_shared<SliceExpr>(slice_end->location, nullptr,
                                              std::move(slice_end));
        } else {
          ...
          if (!consumeToken(":").empty()) {
            consumeSpaces();
            if (peekSymbols({"]"})) {
              index = std::make_shared<SliceExpr>(
                  slice_start->location, std::move(slice_start), nullptr);
            } else {
              auto slice_end = parseExpression();
              index = std::make_shared<SliceExpr>(slice_start->location,
                                                  std::move(slice_start),
                                                  std::move(slice_end));
            }
          } else {
    ...
    try {
      while (it != end) {
        ...
        if (!(group = consumeTokenGroups(comment_tok, SpaceHandling::Keep))
          ...
          tokens.push_back(std::make_unique<CommentTemplateToken>(
          ...
          tokens.push_back(std::make_unique<ExpressionTemplateToken>(
          ...
          if (keyword == "if") {
            ...
            tokens.push_back(std::make_unique<IfTemplateToken>(
            ...
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<ElifTemplateToken>(
                location, pre_space, post_space, std::move(condition)));
          } else if (keyword == "else") {
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<ElseTemplateToken>(
                location, pre_space, post_space));
          } else if (keyword == "endif") {
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<EndIfTemplateToken>(
                location, pre_space, post_space));
          } else if (keyword == "for") {
            ...
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<ForTemplateToken>(
            ...
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<SetTemplateToken>(
                location, pre_space, post_space, ns, var_names,
                std::move(value)));
          } else if (keyword == "endset") {
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<EndSetTemplateToken>(
                location, pre_space, post_space));
            ...
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<MacroTemplateToken>(
                location, pre_space, post_space, std::move(macroname),
                std::move(params)));
          } else if (keyword == "endmacro") {
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<EndMacroTemplateToken>(
                location, pre_space, post_space));
            ...
            auto post_space = parseBlockClose();
            tokens.push_back(std::make_unique<FilterTemplateToken>(
  ...
  std::shared_ptr<TemplateNode> parseTemplate(
    ...
    while (it != end) {
      ...
      } else if (auto for_token =
                     dynamic_cast<ForTemplateToken*>(token.get())) {
      ...
      } else if (auto expr_token =
                     dynamic_cast<ExpressionTemplateToken*>(token.get())) {
        children.emplace_back(std::make_shared<ExpressionNode>(
            token->location, std::move(expr_token->expr)));
      } else if (auto set_token =
                     dynamic_cast<SetTemplateToken*>(token.get())) {
        if (set_token->value) {
      ...
      } else if (auto macro_token =
                     dynamic_cast<MacroTemplateToken*>(token.get())) {
        auto body = parseTemplate(begin, it, end);
      ...
      } else if (auto filter_token =
                     dynamic_cast<FilterTemplateToken*>(token.get())) {
        auto body = parseTemplate(begin, it, end);
...
inline std::shared_ptr<Context> Context::builtins() {
  ...
  globals.set("lower", simple_function(
                           "lower", {"text"},
                           [](const std::shared_ptr<Context>&, Value& args) {
                             auto text = args.at("text");
                             if (text.is_null())
      ...
      simple_function(
          ...
          [](const std::shared_ptr<Context>&, Value& args) {
            ...
            if (args.contains("items")) {
              ...
              return simple_function(
                  "", {"items"},
                  [sep, do_join](const std::shared_ptr<Context>&, Value& args) {
                    auto& items = args.at("items");
  ...
  globals.set(
      "map", Value::callable([=](const std::shared_ptr<Context>& context,
        ...
        if (args.args.size() == 1 &&
          ...
          if (fn.is_null())
            throw std::runtime_error("Undefined filter: " +
                                     args.args[1].dump());
