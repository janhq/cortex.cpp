...
void whisper_print_usage(int /*argc*/, char **argv, const whisper_params &params)
{
    ...
    fprintf(stderr, "  --convert,                     [%-7s] Convert audio to WAV, requires ffmpeg on the server", params.ffmpeg_converter ? "true" : "false");
    fprintf(stderr, "\n");
}
...
std::string whisper_server_context::inference(std::string &input_file_path, std::string language, std::string prompt,
                                              std::string response_format, float temperature, bool translate)
{
    ...
    {
        std::string error_resp = "Failed to execute ffmpeg command converting " + input_file_path + " to wav";
    ...
    if (!whisper_is_multilingual(ctx))
    {
        if (params.language != "en" || params.translate)
        {
            ...
            LOG_WARN << "Model " << model_id << " is not multilingual, ignoring language and translation options";
        }
    }
    ...
    std::string processing_info = "Model " + model_id + "processing " + input_file_path + " (" + std::to_string(pcmf32.size()) + " samples, " + std::to_string(float(pcmf32.size()) / WHISPER_SAMPLE_RATE) + " sec), " + std::to_string(params.n_threads) + " threads, " + std::to_string(params.n_processors) + " processors, lang = " + params.language + ", task = " + (params.translate ? "translate" : "transcribe") + ", " + (params.tinydiarize ? "tdrz = 1, " : "") + (params.no_timestamps ? "timestamps = 0" : "timestamps = 1");
    LOG_INFO << processing_info;

    // run the inference
    {
        std::string msg = "Running whisper.cpp inference of model " + model_id + " on " + input_file_path;
        LOG_INFO << msg;
    ...
    bool translate)
{
    ...
    std::string language = formFields.find("language") != formFields.end() ? formFields.at("language") : "en";
    std::string prompt = formFields.find("prompt") != formFields.end() ? formFields.at("prompt") : "";
    std::string response_format = formFields.find("response_format") != formFields.end() ? formFields.at("response_format") : json_format;
    float temperature = formFields.find("temperature") != formFields.end() ? std::stof(formFields.at("temperature")) : 0;
    ...
    std::string result;
    try {
        result = whispers[model_id].inference(temp_file_path, language, prompt, response_format, temperature, translate);
