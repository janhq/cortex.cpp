...
struct llama_server_context {
  ...
  bool launch_slot_with_data(llama_client_slot *&slot, json data) {
    ...
    if (slot->n_predict > 0 && slot->params.n_predict > slot->n_predict) {
      ...
      LOG_WARNING_LLAMA("Max tokens to predict exceeds server configuration",
                        {
    ...
    if (samplers_sequence != data.end() && samplers_sequence->is_array()) {
      ...
      slot->sparams.samplers_sequence =
          sampler_types_from_names(sampler_names, false);
    } else {
  ...
  void request_completion(int task_id, json data, bool infill, bool embedding,
                          int multitask_id) {
    task_server task;
    task.id = task_id;
    task.target_id = 0;
  ...
  bool ingest_images(llama_client_slot &slot, int n_batch) {
    ...
    while (image_idx < (int)slot.images.size()) {
      ...
      for (int i = 0; i < (int)append_tokens.size(); ++i) {
        llama_batch_add(batch, append_tokens[i],
                        system_tokens.size() + slot.n_past, {slot.id}, true);
        slot.n_past += 1;
      }
    }
  ...
  void split_multiprompt_task(int multitask_id, task_server &multiprompt_task) {
    ...
    for (int i = 0; i < prompt_count; i++) {
      ...
      request_completion(subtask_ids[i], subtask_data,
                         multiprompt_task.infill_mode,
  ...
  void process_single_task(task_server &task) {
    ...
    case TASK_TYPE_SLOTS_DATA: {
      ...
      res.stop = true;
      res.error = false;
      res.result_json = {{"idle", n_idle_slots},
    ...
    for (llama_client_slot &slot : slots) {
      if (slot.ga_n == 1) {
        if (slot.is_processing() &&
            system_tokens.size() + slot.cache_tokens.size() >=
                (size_t)slot.n_ctx) {
          // Shift context
          ...
          llama_kv_cache_seq_shift(ctx, slot.id, n_keep + n_discard,
        ...
        if (slot.state == IDLE && slot.command == LOAD_PROMPT) {
          ...
          for (; slot.n_past < (int)prefix_tokens.size(); ++slot.n_past) {
            ...
            llama_batch_add(batch, prefix_tokens[slot.n_past],
                            system_tokens.size() + slot_npast, {slot.id},
                            false);
            slot_npast++;
          }
      ...
      for (auto &slot : slots) {
        if (slot.ga_n != 1) {
          ...
          while (slot.n_past_se >= slot.ga_i + slot.ga_w) {
            ...
            llama_kv_cache_seq_shift(ctx, slot.id, slot.ga_i, slot.n_past_se,
                                     ib * bd);
            llama_kv_cache_seq_div(ctx, slot.id, slot.ga_i + ib * bd,
                                   slot.ga_i + ib * bd + slot.ga_w, slot.ga_n);
  ...
  void run_on_all_tasks_finished() { update_slots(); }
};
