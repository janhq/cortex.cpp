...
#define LOG_ERROR_LLAMA(MSG, ...) server_log("ERROR", __func__, __LINE__, MSG, __VA_ARGS__)
#define LOG_WARNING_LLAMA(MSG, ...) server_log("WARNING", __func__, __LINE__, MSG, __VA_ARGS__)
#define LOG_INFO_LLAMA(MSG, ...) server_log("INFO", __func__, __LINE__, MSG, __VA_ARGS__)
        ...
        if (!params.grammar.empty()) {
...

            {
                ...
                if (it != params.logit_bias.end() && it->second == -INFINITY) {
                    LOG_WARNING_LLAMA("EOS token is disabled, which will cause most grammars to fail", {});
                }
            }
...
#else
            LOG_WARNING_LLAMA("Not compiled with GPU offload support, --n-gpu-layers option will be ignored. "
...
#else
            LOG_WARNING_LLAMA("llama.cpp was compiled without cuBLAS. It is not possible to set a main GPU.", {});
#endif
        }
        else if (arg == "--lora")
        {
...
public:
  llamaCPP() {
    ...
    LOG_INFO_LLAMA("build info", {{"build", BUILD_NUMBER}, {"commit", BUILD_COMMIT}});
