...
struct whisper_server_context
{
    ...
    std::string inference(std::string &input_file_path, std::string languague, std::string prompt,
...
private:
    ...
    std::optional<std::string>parse_model_id(const std::shared_ptr<Json::Value> &jsonBody,
                                             const std::function<void(const HttpResponsePtr &)> &callback);

    void transcription_impl(const HttpRequestPtr &req,
                            std::function<void(const HttpResponsePtr &)> &&callback,
                            bool translate);
};
